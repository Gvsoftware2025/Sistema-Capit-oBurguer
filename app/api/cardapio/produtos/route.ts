...
export async function POST(request: Request) {
  try {
    ...
    const { category_id, name, description, subcategory, price, image_url, is_available } = body

    const [produto] = await query<DbProduct>(
      `INSERT INTO ${SCHEMA}.products (category_id, name, description, subcategory, price, image_url, is_available)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`,
      [category_id, name, description || null, subcategory || null, price, image_url || null, is_available ?? true]
    )
